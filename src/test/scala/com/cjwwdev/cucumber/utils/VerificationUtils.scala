...
trait VerificationUtils extends Matchers {
  ...
  def verifyPageByTitle(id: String, title: String): Assertion = driver.findElement(By.id(id)).getText shouldBe title

  def verifyTextContains(id: String, contains: String): Assertion = assert(driver.findElement(By.id(id)).getText.contains(contains))
  ...
  def verifyTextDoesNotEqual(id: String, text: String): Assertion = assert(!driver.findElement(By.id(id)).getText.equals(text))

  def verifyDivHasClass(id: String, `class`: String): Assertion = assert(driver.findElement(By.id(id)).getAttribute("class").contains(`class`))
}
